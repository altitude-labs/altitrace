...
import { AltitraceClient, ValidationError, AltitraceApiError } from '@altitrace/sdk';
...
async function basicSimulationExample(): Promise<void> {
  ...
  try {
    ...
    result.calls.forEach((call, index) => {
      ...
      console.log(`    Gas Used: ${parseInt(call.gasUsed, 16).toLocaleString()}`);
...
async function simpleCallExample(): Promise<void> {
  ...
  try {
    ...
    console.log('❌ Simple call failed:', error instanceof Error ? error.message : String(error));
  }
}
...
async function runExamples(): Promise<void> {
  ...
  console.log('\nNext steps:');
  console.log('- Check out advanced-usage.ts for more complex examples');
  console.log('- Read the SDK documentation for full API reference');
  console.log('- Try modifying these examples with your own transactions');
}
