...
import { PlusIcon, PlayIcon, ShareIcon, TrashIcon, ClockIcon } from 'lucide-react';
...
export default function SimulatorDashboard() {
  ...
  const getSimulationType = (request: StoredSimulation['request']) => {
    ...
    const hasValue = request.params.calls?.some(call => call.value && call.value !== '0x0');
    
    if (callsCount > 1) return { label: 'Batch', color: 'bg-purple-100 text-purple-800' };
    if (hasValue) return { label: 'Transfer', color: 'bg-blue-100 text-blue-800' };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Build, simulate, and analyze HyperEVM transactions with professional tooling
            </p>
          </div>
          ...
          {simulations.length === 0 ? (
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-2">
                    Create your first transaction simulation to get started with HyperEVM analysis
                  </p>
                </div>
                <Button onClick={() => router.push('/simulator/new')} className="mt-4">
                  <PlusIcon className="w-4 h-4 mr-2" />
            ...
            <div className="space-y-3">
              ...
              {simulations.map((simulation) => (
                <Card key={simulation.id} className="p-4 hover:bg-muted/50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${getSimulationType(simulation.request).color}`}>
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        {simulation.request.params.blockNumber && (
                          <div>
                            Block: {typeof simulation.request.params.blockNumber === 'string' && simulation.request.params.blockNumber.startsWith('0x') 
                              ? parseInt(simulation.request.params.blockNumber, 16).toLocaleString()
                              : simulation.request.params.blockNumber}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => router.push(`/simulator/${simulation.id}`)}
                      >
        ...
        {simulations.length > 0 && (
          <div className="flex justify-center pt-4">
            <Button variant="outline" onClick={() => router.push('/simulator/new')}>
              <PlusIcon className="w-4 h-4 mr-2" />
