...
import { ArrowLeftIcon, ShareIcon, DownloadIcon, ClockIcon, EditIcon } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const loadAndExecuteSimulation = async () => {
      ...
      try {
        ...
        const storedSimulation = SimulationStorage.retrieveById(resolvedParams.id);
        ...
        const client = new AltitraceClient({ 
          baseUrl: process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/v1` : undefined 
        });
        
        const result = await executeEnhancedSimulation(client, storedSimulation.request);
