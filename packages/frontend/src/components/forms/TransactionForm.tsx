...
import { Card, CardHeader, CardTitle, CardContent, Input, Select, Button, Alert, AlertDescription } from '@/components/ui';
import type { Address, HexString as Hex } from '@altitrace/sdk';
import type { SimulationRequest, SimulationParams, BlockTag } from '@altitrace/sdk';
import type { ParsedAbi } from '@/types/api';
import { validateAddress, validateOptionalHex, validateValue, ValidationError } from '@/utils/validation';
...
export function TransactionForm({ onSubmit, loading, functionData, initialData, compact = false }: TransactionFormProps) {
  const [formData, setFormData] = useState<FormData>(() => ({
    ...initialFormData,
    ...initialData
  }));
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [useBlockNumber, setUseBlockNumber] = useState(!!initialData?.blockNumber);
    ...
    const request: SimulationRequest = {
      params: {
        calls: [{
          ...
          ...(formData.value && formData.value !== '0x0' && { value: formData.value as Hex }),
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className={compact ? "space-y-3" : "space-y-6"}>
          ...
          <div className={compact ? "space-y-2" : "space-y-4"}>
            {!compact && <h3 className="font-semibold text-lg">Transaction Parameters</h3>}
            ...
            <Input
              ...
              error={errors.to}
              description={compact ? undefined : "Contract or wallet address to send transaction to"}
              required
            />
            ...
            <Input
              ...
              error={errors.from}
              description={compact ? undefined : "Address to simulate transaction from (account impersonation)"}
            />
            ...
            <Input
              ...
              error={errors.data}
              description={compact ? undefined : (functionData ? 
            ...
            <Input
              ...
              error={errors.value}
              description={compact ? undefined : "Amount of HYPE to send with transaction (in wei, hex format)"}
            />
            ...
            {showAdvanced && (
              <Input
                ...
                error={errors.gas}
                description={compact ? undefined : "Gas limit for transaction (will be estimated if not provided)"}
              />
            )}
          </div>
          ...
          {showAdvanced && (
            <div className={compact ? "space-y-2" : "space-y-4"}>
              {!compact && <h3 className="font-semibold text-lg">Block Parameters</h3>}
              ...
              {useBlockNumber ? (
                <Input
                  ...
                  description={compact ? undefined : "Specific block number in hex format (e.g., 0x123abc)"}
                />
              ) : (
                <Select
                  ...
                  onChange={(e) => handleInputChange('blockTag', e.target.value as BlockTag)}
                  description={compact ? undefined : "Which block to simulate against"}
                />
              )}
            </div>
          )}
          ...
          {showAdvanced && (
            <div className={compact ? "space-y-2" : "space-y-4"}>
              {!compact && <h3 className="font-semibold text-lg">Simulation Options</h3>}
                ...
                <div className={compact ? "space-y-1" : "space-y-2"}>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => handleInputChange('validation', e.target.checked)}
                      className="rounded"
                    />
          ...
          {!compact && (
            <div className="bg-muted/50 p-4 rounded-lg">
              <h3 className="font-semibold text-sm mb-2">🔍 Auto-enabled Tracing Features</h3>
              <div className="text-xs text-muted-foreground space-y-1">
                <p>• <strong>Asset Changes:</strong> ERC-20/ERC-721 token balance tracking</p>
                <p>• <strong>HYPE Transfers:</strong> Native HYPE movement detection</p>
                <p>• <strong>Call Hierarchy:</strong> Full call tree (when trace API available)</p>
                <p>• <strong>Gas Profiling:</strong> Detailed gas consumption analysis</p>
              </div>
            </div>
          )}
