 ...
 */
export function DecHexToggle({ value, className = '', label, showLabel = true }: DecHexToggleProps) {
 ...
 */
export function DecHexDisplay({ value, format = 'hex' }: { value: string; format?: 'hex' | 'dec' }) {
  const getDisplayValue = () => {
    if (!value || value === '0x' || value === '0x0') {
