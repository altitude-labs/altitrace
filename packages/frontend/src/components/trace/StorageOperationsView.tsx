...
export function StorageOperationsView({
  ...
  const operationsByContract = operations.reduce(
    (acc, op) => {
      if (!acc[op.contract]) {
        acc[op.contract] = []
      }
  ...
  const filteredOperations = operations.filter(
    (op) => filter === 'all' || op.opcode === filter,
  )
        ...
        <CardContent className="p-6 text-center text-muted-foreground">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm max-w-md mx-auto">
              No SSTORE or SLOAD operations were detected in this trace. This
              means no contract storage was read from or written to during
              execution.
            </p>
          </div>
        </CardContent>
    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-3">
        ...
        {Object.entries(operationsByContract).map(([contract, contractOps]) => {
          const filteredContractOps = contractOps.filter(
            (op) => filter === 'all' || op.opcode === filter,
          )
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 space-y-2 min-w-0">
              ...
              <div className="flex items-center gap-1">
                ...
                {operation.value && (
                  <Button
                    ...
                    className="h-6 w-6 p-0"
                    onClick={() =>
                      copyToClipboard(operation.value!, `value-${index}`)
                    }
                  >
                    <CopyIcon className="h-3 w-3" />
                  </Button>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button
                  ...
                  className="h-6 w-6 p-0"
                  onClick={() =>
                    copyToClipboard(operation.value!, `sload-value-${index}`)
                  }
                >
                  <CopyIcon className="h-3 w-3" />
                </Button>
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            ...
            <span>
              Call: [{operation.callContext || `${operation.depth}.0`}]
            </span>
          </div>
        </div>
      </div>
