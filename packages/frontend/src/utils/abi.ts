...
import type { AbiFunction, AbiParameter, ParsedAbi } from '@/types/api'
...
export function parseAbiJson(abiJson: string): ParsedAbi {
  try {
    ...
    throw error instanceof AbiError
      ? error
      : new AbiError('Failed to parse ABI')
  }
}
...
export function validateParameter(
  ...
  try {
    ...
    if (type.match(/^u?int(\d+)?$/)) {
      ...
      const bitSize = bits ? Number.parseInt(bits) : 256
      ...
      try {
        ...
        const minValue = isUnsigned ? 0n : -(2n ** (BigInt(bitSize) - 1n))
...
export function encodeFunctionCall(
  ...
  try {
    ...
    const args = (func as unknown as { inputs: AbiParameter[] }).inputs.map(
      (input: AbiParameter) => {
