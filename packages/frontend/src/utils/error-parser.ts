...
export function parseBlockchainError(
  ...
  const errorMessage =
    typeof error === 'string' ? error : error.reason || error.message || ''

  // Insufficient funds error pattern
  const insufficientFundsMatch = errorMessage.match(
    /lack of funds\s*\((\d+)\)\s*for max fee\s*\((\d+)\)/i,
  )
  ...
  const gasLimitMatch = errorMessage.match(
    /gas limit reached|out of gas|exceeds block gas limit/i,
  )
  if (gasLimitMatch) {
    return {
      type: 'gas-limit',
  ...
  if (errorMessage.includes('RPC') || errorMessage.includes('error code')) {
    ...
    const rpcMatch = errorMessage.match(
      /EVM reported invalid transaction[^:]*:\s*(.+)/,
    )
    if (rpcMatch) {
    ...
    return {
      ...
      title: 'RPC Error',
      details: errorMessage
        .replace(/RPC internal error:\s*/i, '')
 ...
 */
export function parseBlockchainErrorWithOutput(
  error: string | { reason?: string; message?: string },
  outputData?: string,
): ParsedError {
